...
function respond() {
  ...
  if (request.text && (request.text.indexOf("!rd") == 0)) {
    ...
    sender(request.text, request.name);
    this.res.end();
  } else if(request.text && (request.text.indexOf("!social") == 0)) {
    this.res.writeHead(200);
    socialLink();
    this.res.end();
...
function sender(tempTxt, name) {
  ...
  botResponse = name + ": " + tempTxt.substring(tempTxt.indexOf("d") + 1, tempTxt.length);
...
function socialLink() {
  ...
  botResponse = "Here is the link: https://bit.ly/2FMwDxm";
